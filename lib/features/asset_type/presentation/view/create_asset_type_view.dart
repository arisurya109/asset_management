        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              BlocConsumer<AssetTypeBloc, AssetTypeState>(
                listener: (context, state) {
                  nameC.clear();
                  initC.clear();
                  if (state.status == StatusAssetType.failed) {
                    context.showSnackbar(
                      state.message ?? 'Failed to create asset types',
                      backgroundColor: AppColors.kRed,
                    );
                  }

                  if (state.status == StatusAssetType.success) {
                    context.showSnackbar('Successfully create asset types');
                  }
                },
                builder: (context, state) {
                  return AppButton(
                    title: state.status == StatusAssetType.loading
                        ? 'Loading...'
                        : 'Create',
                    width: double.maxFinite,

                    onPressed: state.status == StatusAssetType.loading
                        ? null
                        : _onSubmit,
                  );
                },
      ...
      context.showDialogConfirm(
        ...
        onConfirm: () {
          context.read<AssetTypeBloc>().add(
            OnCreateAssetType(AssetType(name: name, init: init)),
          );
